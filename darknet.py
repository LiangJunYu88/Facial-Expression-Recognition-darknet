...
if os.name == "nt":
    ...
    lib = CDLL("./FER2013_VGG19/libdarknet.so", RTLD_GLOBAL)
    ...
    for b in range(batch_size):
        ...
        for i in range(num):
            ...
            if score > thresh:
                ...
                # show the score
                image_list[b] = cv2.putText(image_list[b], str(score), (right + 2, top + 2), cv2.FONT_HERSHEY_SIMPLEX, 1.2, boxColor, 2)
...
def testV4Person(path):
    if not os.path.exists(path):
        return
    for aroot, _, files in os.walk(path):
        for item in files:
            if (not item.endswith(".jpg")) and (not item.endswith(".png")):
                continue
            file_path = os.path.join(aroot, item)
            print(performDetect(file_path, 0.2, "cfg/yolov4.cfg", "pre-trained/yolov4.weights", "data/person_zs622/cfg/person_zs622.data"))

def testKinthPerson(path):
    if not os.path.exists(path):
        return
    for aroot, _, files in os.walk(path):
        for item in files:
            if (not item.endswith(".jpg")) and (not item.endswith(".png")):
                continue
            file_path = os.path.join(aroot, item)
            print(performDetect(file_path, 0.2, "cfg/yolov4-kinth-person.cfg", "trained/yolov4-kinth-person_best_20200704.weights", "data/person_zs622/cfg/person_zs622.data"))

if __name__ == "__main__":
    testPath = "../data/test"
    #testV4Person(testPath)
    testKinthPerson(testPath)
